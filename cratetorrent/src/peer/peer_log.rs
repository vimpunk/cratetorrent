...
macro_rules! peer_error {
    ($self:ident, $($arg:tt)*) => ({
        ::log::error!("[{}] {}", $self.peer.addr, format!($($arg)*));
    })
}

macro_rules! peer_warn {
    ($self:ident, $($arg:tt)*) => ({
        ::log::warn!("[{}][{}] {}", $self.torrent.id, $self.peer.addr, format!($($arg)*));
    })
}

macro_rules! peer_info {
    ($self:ident, $($arg:tt)*) => ({
        ::log::info!("[{}][{}] {}", $self.torrent.id, $self.peer.addr, format!($($arg)*));
    })
}

macro_rules! peer_debug {
    ($self:ident, $($arg:tt)*) => ({
        ::log::debug!("[{}][{}] {}", $self.torrent.id, $self.peer.addr, format!($($arg)*));
    })
}

macro_rules! peer_trace {
    ($self:ident, $($arg:tt)*) => ({
        ::log::trace!("[{}][{}] {}", $self.torrent.id, $self.peer.addr, format!($($arg)*));
    })
}
