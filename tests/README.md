...
- **Goal**: the successful upload of an arbitrary torrent file from a single
  cratetorrent seed
- **Note**: Since cratetorrent currently can't create a metainfo file, we
  bootstrap this test with transmission's equivalent functionality. Therefore
  the metainfo and source files have to be generated by another test run (any of
  the above), and the name of the torrent has to be passed to the test. It will
  then lookup the existing torrent in the `assets` directory.
- **Command**:
  [`./test_seed_single_conn.sh --torrent-name TORRENT`](test_seed_single_conn.sh)
- **Containers**:
  - *seeds:*
